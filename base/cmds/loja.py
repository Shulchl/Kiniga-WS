...
class Shop(commands.Cog):
    ...
    async def comprar(self, interaction: Interaction, item_id: int) -> None:
        ...
        items_ids_ = []
        for i, items in enumerate(user_itens):
            list_key, list_values = items[0], items[1]
            if list_key == ivent_key_name:
                for k, v in list_values.items():
                    items_ids_.append(k)

        if str(item_id_uui) in items_ids_:
        ...
        l = await user_inventory(self, interaction.user.id, 'remove', str(ivent_key_name), item_id_uui)
    ...
    async def desequipar(self, interaction: Interaction, item_id: int):
        ...
        try:
            ...
            await interaction.response.send_message(
                "%s %s %s!" %
                (type_.title(), name, 
                    "deequipada" if type_.endswith('a') else "equipado"), 
                ephemeral=True
            )
    @desequipar.error
