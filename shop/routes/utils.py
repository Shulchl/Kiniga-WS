...
@app.route("/")
async def index():
    if not await discord_.authorized:
        return f"""
...
@app.route("/login/")
async def login():
    return await discord_.create_session()


@app.route("/login-data/")
async def login_with_data():
    return await discord_.create_session(data=dict(redirect="/me/", coupon="15off", number=15, zero=0, status=False))
...
async def invite_oauth():
    return await discord_.create_session(scope=["bot", "identify"], permissions=8)


@app.route("/callback/")
async def callback():
    data = await discord_.callback()
...
async def user_guilds():
    guilds = await discord_.fetch_guilds()
...
async def my_connections():
    user = await discord_.fetch_user()
    connections = await discord_.fetch_connections()
    return f"""
<html>
<head>
<title>{user.name}</title>
